...
async function getAllPosts(apolloClient, process, verbose = false) {
  ...
  try {
    ...
    const nodes = [...data.data.allWpPost.edges.map(({ node = {} }) => node)];
    ...
    posts = nodes.map((post) => {
      ...
      if (data.categories) {
        data.categories = data.categories.nodes.map(({ name }) => name);
      }
...
async function getSiteMetadata(apolloClient, process, verbose = false) {
  ...
  try {
    const data = await apolloClient.query({ query });
    metadata = { ...data.data.wp.generalSettings };
...
async function getPages(apolloClient, process, verbose = false) {
  ...
  try {
    const data = await apolloClient.query({ query });
    pages = [
      ...data.data.allWpPage.nodes.map((node) => {
        return {
          slug: node.slug,
