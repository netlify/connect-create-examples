...
class WebpackPlugin {
  ...
  async index(compilation, options) {
    ...
    try {
      ...
      if (!hasUrl) {
        throw new Error(
          `[${plugin.name}] Failed to compile: Please check that NETLIFY_CONNECT_ENDPOINT is set and configured in your environment. WORDPRESS_HOST is no longer supported by default.`
        );
      }
