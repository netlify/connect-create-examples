...
export async function getPostBySlug(slug) {
  ...
  const apiHost = new URL(process.env.NETLIFY_CONNECT_ENDPOINT).host;
  ...
  if (!postData?.data.wpPost && !postData?.data?.contentfulPageBlogPost) return { post: undefined };

  const post = [postData?.data.wpPost || postData?.data?.contentfulPageBlogPost].map(mapPostData)[0];
...
export async function getPostsByCategoryId({ categoryId, ...options }) {
  ...
  const posts = postData?.data.allWpPost.edges.map(({ node = {} }) => node);
  return {
    posts: Array.isArray(posts) && posts.map(mapPostData),
  };
}
...
export function mapPostData(post = {}) {
  ...
  if (data.excerpt || data?.shortDescription?.shortDescription) {
    data.excerpt = post.excerpt || post?.shortDescription?.shortDescription;
  }

  if (data.date || data.publishedDate) {
    data.date = post.date || post.publishedDate;
  }

  if (data.modified || data.updatedAt) {
    data.modified = post.modified || post.updatedAt;
  }
  ...
  if (data.author) {
    data.author = {
      ...(data.author.node || data.author),
    };
  }

  if (data.content?.raw) {
    data.content = documentToHtmlString(JSON.parse(data.content.raw));
  }
  ...
  if (data.featuredImage) {
    data.featuredImage = data.featuredImage.node || data.featuredImage;
    data.featuredImage = {
      altText: data.featuredImage.altText || data.featuredImage.description,
...
export async function getRelatedPosts(categories, postId, count = 5) {
  ...
  if (!Array.isArray(related.posts) || related.posts.length === 0) {
    const relatedPosts = await getRelatedPosts(newCategoriesArr, postId, count);
...
export async function getPostsPerPage() {
  ...
  try {
    ...
    return Number(data.wp.allSettings.readingSettingsPostsPerPage);
  } catch (e) {
